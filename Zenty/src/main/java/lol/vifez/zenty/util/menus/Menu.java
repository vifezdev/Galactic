package lol.vifez.zenty.util.menus;
...
public abstract class Menu {
    ...
    static {
        Zenty.getInstance().getServer().getPluginManager().registerEvents(new ButtonListener(), Zenty.getInstance());
        Menu.currentlyOpenedMenus = new HashMap<>();
        Menu.checkTasks = new HashMap<>();
        Menu.openInventoryMethod = getOpenInventoryMethod(); // Ensure openInventoryMethod is set during initialization
    }
    ...
    public void openMenu(final Player player) {
        ...
        try {
            getOpenInventoryMethod().invoke(player, inv, 0);
            this.update(player);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    private void update(final Player player) {
        ...
        runnable.runTaskTimer(Zenty.getInstance(), 0L, 0L);
